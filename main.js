...
class IpAddress {
   ...
   * @return {Object} (ipv4:firstIpAddress,ipv6:ipv6MappedAddress) - An IPv4 address.
   */
  getFirstIpAddress(cidrStr, callback) {
 
    // Initialize return arguments for callback
    let callbackError = null;
    let response = {ipv4:null, ipv6:null};
    ...
    if (!cidr.isValid()) {
      ...
      let endAddress = null;
      [startAddress, endAddress] = cidr.toRange()
      if (startAddress == endAddress) {
        callbackError = 'Error: CIDR passed to getFirstIpAddress specifies exactly one address; there is no "first"'
      } else {
        // If the passed CIDR is valid, call the object's toArray() method.
        // Notice the destructering assignment syntax to get the value of the first array's element.
        [response.ipv4] = cidr.toArray(options);
        if (typeof response.ipv4 !== 'undefined') {
          response.ipv6 = getIpv4MappedIpv6Address(response.ipv4)
        } else {
          callbackError = 'Error: Invalid response from ip-cidr; data issue?';
        }
      }
    }
    ...
    return callback(response, callbackError);
  }
}
